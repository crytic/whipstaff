...
\*        - validator_init_done: a tuple indicating whether the validator has sent the initial message (1 is done)
...
\*        Finds the existence of an e-clique
        e_clique_estimate_safety(estimate, messages) == 
            /\ \E ss \in e_clique(estimate, messages):
                /\ 2 * Sum({validator_weights[v] : v \in ss}) > Sum({validator_weights[v] : v \in validators}) + byzantine_threshold - 2 * fault_weight(messages)
...
\* Last modified Fri Aug 09 11:37:59 PDT 2019 by anneouyang
